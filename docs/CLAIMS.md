...
| History cannot be silently rewritten | Consistency proofs between tree sizes; **equivocation detection**; **external anchoring** of signed tree heads via an append-only hash chain, plus **OpenTimestamps** (Bitcoin) via the reference `ots` client | ✅ M4 |
...
| It actually works | Multi-chunk (200 KB) round-trip; **106 tests** incl. property tests; `cargo xtask demo` end-to-end | ✅ M4 |

...
/// Derive a short recipient tag for a drop.
///
/// The tag lets a recipient find the sealed key addressed to them **without**
/// the drop naming anyone in the clear. `drop_nonce` is per-drop, so tags are
/// unlinkable across drops. Decoy tags (generated by the sender) create false
/// positives and hide the true recipient count.
///
/// # Errors
/// Propagates [`CryptoError::InvalidKeyLength`].
pub fn derive_tag(
    ...
    let mut info = Vec::with_capacity(INFO_TAG.len() + 16);
    info.extend_from_slice(INFO_TAG);
    info.extend_from_slice(drop_nonce);
    let key = derive_key32(recipient_public, DEFAULT_SALT, &info)?;
    let mut tag = [0u8; 8];
    tag.copy_from_slice(&key[..8]);
    Ok(tag)
}
...
mod tests {
    ...
    #[test]
    fn tags_depend_on_nonce_and_recipient() {
        let recipient = [1u8; KEY_LEN];
        let other = [2u8; KEY_LEN];
        let nonce_a = [9u8; 16];
        let nonce_b = [10u8; 16];
        let t1 = derive_tag(&recipient, &nonce_a).unwrap();
        assert_eq!(t1, derive_tag(&recipient, &nonce_a).unwrap());
        assert_ne!(t1, derive_tag(&recipient, &nonce_b).unwrap());
        assert_ne!(t1, derive_tag(&other, &nonce_a).unwrap());
    }
}
